...
import getProductByFile from "../../../../api/getProductByFile";
import getProductByFileTxT from "../../../../api/getProductByFileTxT";
...
import ExportDriverTypeCylinderXuanNghiem from "../factory/ExportDriverTypeCylinderXuanNghiem";
...
import FinishXuanNghiemTurnBack from "../factory/FinishXuanNghiemTurnBack";
...
import ImportWarehouseTranfer from "../factory/ImportWarehouseTransfer";
import ImportTurnBackVehicle from "../factory/ImportTurnBackVehicle";
import GetHistoryDate from "../../../../api/GetHistoryDate";
import GetSearchAllHistoryList from "../../../../api/GetSearchAllHistoryList";
import moment from "moment";
import { saveAs } from 'file-saver';
const ExcelJS = require('exceljs');
import getAllHistoryList from "getAllHistoryList";
...
class Product extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			loading: false,
			//show modal view,
			modalView: 0, // === 1 open modal info product
			historyCylinderAllList: [],
			searchKeyTxT:"",
			searchAll: ""
		};
	}
	...
	handleClickNextOrPreButton(type) {
		...
		let { currentSkipTxT, searchKeyTxT } = this.state;
		if (searchKeyTxT !== "") {
			if (type === 0) {
				currentSkipTxT--;
				if (currentSkipTxT < 0) {
			...
			this.setState({ currentSkipTxT });
			this.searchCylinderTxt(searchKeyTxT, currentSkipTxT);
		}

	}
async searchCylinderTxt(serial, page) {
	//showToast(serial);

	const dataProducts = await getProductByFile(serial, page);
...
showFile = async (e) => {
	...
	let elm = document.getElementById("imp_search")
	exampleFileReader.onload = async (e) => { 
		let text = (e.target.result).replace(/\s/g, '|')
		// return text;
		this.setState({
			searchKeyTxT:text,
			searchKey:"",
...
// async handleClickSearchTxT(page = this.state.activePage) {
// 	this.setState({
// 		loading: true,
// 	});
// 	if (this.state.searchKeyTxT==="" ) {
// 		showToast("Vui lòng nhập hoặc import file")
// 		this.setState({
// 			loading: true,
// 		});
...
// 	} else  {
// 		await this.setState({
// 				searchKeyTxT: this.state.searchKeyTxT,
// 				currentSkip: 1,
// 				loading: true,
// 				searchKey:""
// 			});
// 		await this.search_keyRef.value === ""
// 		await this.searchCylinderTxt(this.state.searchKeyTxT, page);
...
// }
	...
	async handleClickSearch(page = this.state.activePage) {
		...
		if (this.search_keyRef.value === "" && this.state.searchKeyTxT === "") {
			showToast("Hãy nhập mã serial để quét");
			this.setState({
				loading: false,
			});
		} else if(!this.state.searchKeyTxT) {
			await this.setState({
				searchKey: this.search_keyRef.value,
				currentSkip: 1,
			});
			await this.searchCylinder(this.state.searchKey, page);
		}else{
			await this.setState({
				searchKeyTxT: this.state.searchKeyTxT,
				currentSkip: 1,
	...
	async searchCylinder(serial, page) {
		...
		const dataProducts = await GetSearchAllHistoryList(serial, page);
		if (dataProducts) {
			if (
				dataProducts.status === Constants.HTTP_SUCCESS_BODY &&
				!dataProducts.data.hasOwnProperty("err_msg")
		...
		return (
			<td className="text-center table-actions">
				<a
					...
					onClick={() => {
						this.setState({ product_id: store.id, store, modalView: 0});
					}}
				>
				...
				<a
					className="table-action hover-primary"
						...
						data-target="#edit-product"
						onClick={() => {
						this.setState({ product_id: store.id, store, modalView: 1});
					}}
				>
	...
	async getAllListCylinder(){
		let data = await getAllHistoryList(this.state.searchKey, this.state.total);
		if(data){
			this.setState({historyCylinderAllList: data.data, loading: false,})
		}else{
			showToast("Xảy ra lỗi trong quá trình lấy dữ liệu");
		}
	}
...
async getAllListCylinderTxT(){
		let data = await getProductByFileTxT(this.state.searchKeyTxT, this.state.total);
		if(data){
			this.setState({historyCylinderAllList: data.data.data, loading: false,})
		}else{
			showToast("Xảy ra lỗi trong quá trình lấy dữ liệu");
		}
	}
	handleExportFileToExcel = async() =>{
		this.setState({loading: true});
		if(this.state.dataProducts.length > 0){
			if(this.state.searchKey){
				await this.getAllListCylinder();
				
			}
			else{
				await this.getAllListCylinderTxT()
			}
			const workbook = new ExcelJS.Workbook();
  			const worksheet = workbook.addWorksheet("Danh sách sản phẩm");
			worksheet.columns = [
				{header: 'STT', key: 'id', width: 10},
				{header: 'Mã bình', key: 'serial', width: 15}, 
				{header: 'Loại bình', key: 'cylinderType', width: 10,},
				{header: 'Màu sắc', key: 'color', width: 10},
				{header: 'Loại van', key: 'valve', width: 10},
				{header: 'Cân nặng', key: 'weight', width: 10},
				{header: 'Ngày kiểm định', key: 'dateTest', width: 20},
				{header: 'Ngày xuất kho gần nhất', key: 'dateExport', width: 25},
				{header: 'Số vòng đời', key: 'countCircle', width: 15},
				{header: 'Vị trí', key: 'address', width: 40},
				{header: 'Thương hiệu', key: 'manufactory', width: 30}
			];
				this.state.historyCylinderAllList.map((data, index) => {
					worksheet.addRow({
						id: index + 1,
						serial: data.serial ? data.serial : "unknown", 
						cylinderType:  data.cylinderType ?(data.cylinderType.name !== null ? data.cylinderType.name : "unknown"): "unknown" , 
						color: data.color ? data.color : "unknown", 
						valve: data.valve ? data.valve : "unknown", 
						weight:data.weight ? data.weight : "unknown", 
						dateTest: moment(data.checkedDate).format("DD/MM/YYYY"),
						dateExport: typeof data.histories === "undefined" ? "null" : data.histories.length > 0 
										? moment(data.histories[0].createdAt).format("DD/MM/YYYY")
										: "null", 
						countCircle: data.circleCount ? data.circleCount : "unknown", 
						address: checkPosition(
								Constant.PLACESTATUS_ENUM,
								data,
								this.state.user_cookies.user.id
							), 
						manufactory: data.manufacture.name ? data.manufacture.name : "unknown"
					});
				})
			
			
			await workbook.xlsx.writeBuffer().then(buffer => FileSaver.saveAs(new Blob([buffer]),"DanhSachSanPham.xlsx"))
			...
			<div className="main-content">
								...
								this.state.user_role !== "Inspector" && (
									<div className="input-group-product">
										<input id="imp_search"
											...
											onChange={()=>{
												this.setState({searchKeyTxT: null})
											}}
										/>
										<div className="import_box">
											<input  className="inputfile" name="file" type="file" accept=".txt,.xls,.xlsx" onChange={(e)=>this.showFile(e)}  title = "Import file ..."/>
										</div>
										<button
										...
										{/* <button
											onClick={() => this.handleClickSearchTxT()}
											className="btn btn-dark ml-3"
										>Tim Theo File</button> */}
										{this.state.total > 0 ? <button className="btn btn-primary w-50 ml-3" onClick={this.handleExportFileToExcel}>Xuất File Excel <i className="ti-export ml-3"></i></button> : <div></div>}
							...
							{this.state.user_type === Constants.FACTORY &&
								...
								(
									<button
										className="btn btn-sm btn-warning kiemdinh"
										data-toggle="modal"
										data-target="#warehouse-transfer"
									>
										Chuyển kho
									</button>
								)}
                          ...
                          <th className="w-70px text-center align-middle">Giá bán</th> */}
												...
												{this.state.dataProducts.map((store, index) => {
													...
													return (
														<tr key={index}>
															...
															<td scope="row" className="text-center">
																{store.cylinderType ? store.cylinderType.name : store.cylinderType}
															</td>
                                                    ...
                                                    <td scope="row" className="text-center">{store.agency?store.agency.name:''}</td>*/}
															...
															<td scope="row" className="text-center">
																{typeof store.histories === "undefined" ? "null" : store.histories.length > 0 ? moment(store.histories[0].createdAt).format("DD/MM/YYYY")  : "null"}
															</td>
															<td scope="row" className="text-center">
																{store.circleCount}
															</td>
							...
							<PaginationComponent
								...
								getDataPage={(page, search) =>{
										if(this.state.searchKey===""){										 
											this.handleClickSearchTxT(page, search)
										}else{
				...
				{((this.state.user_type === "Factory" &&
					...
					(this.state.user_type === "General" &&
						this.state.user_role !== "Deliver" &&
						this.state.user_role !== "Inspector")) && (
						<ExportDriverTypeCylinderXuanNghiem
							product_parse={this.state.product_parse}
							typeExportCylinder={this.state.typeExportCylinder}
							listUsersPartner={this.state.listUsersPartner}
							listUserFixer={this.state.listUserFixer}
