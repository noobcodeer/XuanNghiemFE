...
import FormValve from "./formValve";
...
class Valve extends React.Component{
    ...
    handleCreateValve= async()=>{
       await this.setState({
            isCreateMode:true,
            data:null
        })
    }
    handleUpdateValve=async(value)=>{
        await this.setState({
            isCreateMode:false,
            data:value,
        })
        ...
        if(answer){
            await callApi("DELETE",api,"",token).then(res=>{
                ...
                if(res.status === 200){
                    ...
                    showToast("Xóa loại van thất bại");
                    return false;
                }
            });
        }   
                ...
                <div className="card-title">
                    <div className="flexbox">
                        ...
                        <div className="row">
                            ...
                            <button onClick={this.handleCreateValve} style={{ marginLeft: '20px' }}
                                className="btn btn-sm btn-create" data-toggle="modal"
                                data-target="#create-valve">Tạo Van</button>
                        </div>
                    </div>
                </div>
                                    ...
                                    <table className="table table-striped table-bordered seednet-table-keep-column-width"
                                        ...
                                        <thead className="table__head">
                                            <tr>
                                                ...
                                                <th className="w-120px text-center">Mã van </th>
                                                <th className="w-120px text-center">Tên van</th>
                                                ...
                                                return(
                                                    ...
                                                    <td className="text-center table-actions">
                                                        ...
                                                        // data-target="#view-report"
                                                            onClick={() => { this.deleteValve(data.id) }}
                                                            >
                                                            <i className="ti-trash"></i> </a>
                                                    
                                                    <a className="table-action hover-primary" 
                                                    data-toggle="modal" 
                                                    data-target="#create-valve"
                                                    onClick={()=>this.handleUpdateValve(data)}
