...
class ExportDriverFactoryPopup extends React.Component {
  ...
  handleChangeCustomer = (langValue) => {
    console.log('CustomerResults', langValue)
    ...
    await callApi("POST", GETTYPECUSTOMER, params, token).then((res) => {
      ...
      if (res.data) {
        if (res.data.success === true) {
          ...
          this.setState({
            listAgenLv1: listRestaurantTemp,
          });
        } else {
          showToast(
            res.data.message ? res.data.message : res.data.err_msg,
    ...
    await callApi("POST", GETTYPECUSTOMER, params, token).then((res) => {
      ...
      if (res.data) {
        if (res.data.success === true) {
          ...
          this.setState({
            listAgenLv2: listRestaurantTemp,
          });
        } else {
          showToast(
            res.data.message ? res.data.message : res.data.err_msg,
  ...
  async componentDidMount() {
    ...
    await this.getAgenLv1(id, token);
    await this.getAgenLv2(id, token);
    await this.getAllArea();
  }

   getAllArea = async () => {
    let areas = await getArea('null');
    this.setState({listAreas: areas.data.data});
  }
  ...
  changeArea = (e) => {
    ...
    // Lọc danh từng loại khách hàng (đã có) theo khu vực
    let listAgenLv2ByArea = []
    listAgenLv2ByArea = this.state.listAgenLv2.filter(customer =>
      customer.area && (customer.area.name === e.target.value)
    )
    this.setState({listAgenLv2ByArea: listAgenLv2ByArea})

    let listAgenLv1ByArea = []
    listAgenLv1ByArea = this.state.listAgenLv1.filter(customer =>
      customer.area && (customer.area.name === e.target.value)
    )
    this.setState({listAgenLv1ByArea: listAgenLv1ByArea})

    let listIndustryByArea = []
    listIndustryByArea = this.state.listIndustry.filter(customer =>
      customer.area && (customer.area.name === e.target.value)
    )
    this.setState({listIndustryByArea: listIndustryByArea})
    
    let listDistributionAgencyByArea = []
    listDistributionAgencyByArea = this.state.listDistributionAgency.filter(customer =>
      customer.area && (customer.area.name === e.target.value)
    )
    this.setState({listDistributionAgencyByArea: listDistributionAgencyByArea})
  }
                  ...
                  <div className="row">
                    ...
                    {this.props.userType === Constants.GENERAL ? (
                      ...
                      <div className="col-md-6">
                        ...
                        <div className="form-group">
                          <label>Chọn khu vực 11</label>
                          <select value={this.state.area} className="d-block" onChange={(e) => this.changeArea(e)}>
                            <option key="default" value=''>Vui Lòng Chọn</option>
                            {
                            this.state.listAreas.map((area,index) => 
                        ...
                        {this.state.typeCustomer === "Distribution_Agency" && (
                          <div className="form-group">
                            ...
                            <SelectMulti.Creatable
                              multi={true}
                              options={this.state.listDistributionAgencyByArea}
                          ...
                          {this.state.typeCustomer === "Level_2_Agency" && (
                            <div className="form-group">
                              <label>Chọn các khách hàng</label>
                              <SelectMulti.Creatable
                                multi={true}
                                options={this.state.listAgenLv2ByArea}
                                onChange={this.handleChangeCustomer.bind(this)}
                                placeholder={this.props.t("CHOOSE")}
                                value={this.state.CustomerResults}
                                promptTextCreator={() => {
                          ...
                          {this.state.typeCustomer === "Level_1_Agency" && (
                            <div className="form-group">
                              <label>Chọn các khách hàng</label>
                              <SelectMulti.Creatable
                                multi={true}
                                options={this.state.listAgenLv1ByArea}
                                onChange={this.handleChangeCustomer.bind(this)}
                                placeholder={this.props.t("CHOOSE")}
                                value={this.state.CustomerResults}
                                promptTextCreator={() => {
                          ...
                          {this.state.typeCustomer === "Industry" && (
                            <div className="form-group">
                              ...
                              <SelectMulti.Creatable
                                multi={true}
                                options={this.state.listIndustryByArea}
                                onChange={this.handleChangeCustomer.bind(this)}
                                placeholder={this.props.t("CHOOSE")}
                                value={this.state.CustomerResults}
                                promptTextCreator={() => {
                          ...
                          <div className="form-group">
                            ...
                            >
                              ...
                              {this.state.typeCustomer === "Industry" &&
                                ...
                                if(l.area&&(l.area.name === this.state.area))
                                  return (
                                    <option key={index} value={l.id}>
                                      {l.name}
                                    </option>
                              ...
                              {this.state.typeCustomer ===
                                'Level_1_Agency' &&
                                this.state.listAgenLv1.map((l, index) => {
                                if(l.area&&(l.area.name === this.state.area))
                                  return (
                                    <option key={index} value={l.id}>
                                      {l.name}
                               ...
                               {this.state.typeCustomer ===
                                'Level_2_Agency' &&
                                this.state.listAgenLv2.map((l, index) => {
                                if(l.area&&(l.area.name === this.state.area))
                                  return (
                                    <option key={index} value={l.id}>
                                      {l.name}
