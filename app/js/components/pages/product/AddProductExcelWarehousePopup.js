...
class AddProductExcelWarehousePopup extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      chooseType: [
        ...
        {
          value: 2,
          label: "Tại kho trực thuộc Xuan Nghiem Gas",
          name: "Kho Trực Thuộc Xuan Nghiem Gas",
        },
      ],
      dataChoose: 0,
      ListUserSubmitID: "",
  ...
  async componentDidMount() {
    ...
    await this.getListCylinderType(user_cookies.user.id,token);
  ...
  async getListCylinderType(id,token){

    let api = CYLINDERTYPE+"?id="+id;
    let listCylinderType=[];
    await callApi("GET",api,'',token).then(res=>{
        if(res.status === 200){
          if(res.data.status===true){
            for(let i = 0;i <(res.data.data).length;i++){
                listCylinderType.push({
                    value:(res.data.data)[i].id,
                    label:(res.data.data)[i].name,
                    ...(res.data.data)[i]
                })
  ...
  async getListRentalPartner(id, token) {
    ...
    if (this.state.listPartner) {
      if (this.state.listPartner.status === true) {
        ...
        showToast(
          this.state.listPartner.message,
            // ? this.state.listPartner.message
            // : 
                    ...
                    <div className="col-md-6">
                      <div className="form-group">
                        ...
                        <Select
                          ...
                          options={this.state.listCylinderType}
