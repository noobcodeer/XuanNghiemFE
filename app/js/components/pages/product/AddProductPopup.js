...
class AddProductPopup extends React.Component {
    ...
    selectOptionHandler = (e) => {
        const{listTypeCylinder} =this.props;
        let target = e.target;
        let name = target.name;
        let value = target.value;
        if(name === 'cylinderType'){
            if(value !==''){
                listTypeCylinder.map(todo=>{
                    if(todo.id === value){
                        this.setState({
    ...
    render() {
        ...
        const { value,listValve,listColor } = this.state;
        return (

            <div className="modal fade" id="create-product" tabIndex="-1">
                <div className="modal-dialog modal-lg modal-create-product">
