...
export default function khovo() {
    ...
    const [listStatictis, setListStatictis] = useState({ createdCylinder: { cylinderTypes: [] }, inventoryCylinder: { cylinderTypes: [] }, turnbackCylinderFromTruck: { cylinderTypes: [] } });
    async function getWarehouse() {
        const user_cookies = await getUserCookies();
        const id = user_cookies.user.id;
        const resultWarehouse = await getWarehouseType({ id, userType: "Warehouse_EMPTY" })
    ...
    async function getStatisticware() {
        if (startDate == "" || endDate == "") {
            ...
            const resultStatisticWare = await getStatisticWare({ target: ware, startDate, endDate, statisticalType })
            if (resultStatisticWare.status) {
                // setListStatictis(resultStatisticWare.data.data);
                const newList = {...listStatictis};
                newList.createdCylinder.cylinderTypes = groupByManufacture(resultStatisticWare.data.data.createdCylinder.cylinderTypes,'manufactureId');
                ...
                newList.turnbackCylinderFromTruck.cylinderTypes = groupByManufacture(resultStatisticWare.data.data.turnbackCylinderFromTruck.cylinderTypes,'manufactureId');
                    ...
                    <div className="col-6">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                {listStatictis.createdCylinder.cylinderTypes.map((item, index) => {
                                    let Sum = 0;
                                    // Sum = Sum + item.number;
                                    return <tr key={index}>
                    ...
                    <div className="col-6">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                {listStatictis.turnbackCylinderFromTruck.cylinderTypes.map((item, index) => {
                                    let Sum = 0;
                                    // Sum = Sum + item.number;
                                    return <tr key={index}>
                                        <td>{item.manufactureName}</td>
